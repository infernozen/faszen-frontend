...
class SearchDisplayPage extends StatelessWidget {
  String productName;
  SearchDisplayPage({super.key, required this.productName});

  @override
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => ProductDataProvider(),
      child: SearchDisplay(productName: productName),
    );
  }
}
...
class SearchDisplay extends StatefulWidget {
  ...
  const SearchDisplay({super.key, required this.productName});
...
class _SearchDisplayState extends State<SearchDisplay> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      dataProvider.fetchData(widget.productName);
    });
  }
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        dataProvider.isLoading
            ...
            : dataProvider.productList.isNotEmpty
                ? Expanded(
                    child: SizedBox(
                      height: dataProvider.productList.length * 100,
                      child: ListView.builder(
                        scrollDirection: Axis.vertical,
                        itemCount: dataProvider.productList.length,
                        itemBuilder: (context, index) {
                          var product = dataProvider.productList[index];
                          if(index == 0){
                            return Padding(
